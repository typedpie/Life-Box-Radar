...
class BigQueryClient:
    def __init__(self, project_id, dataset_id, table_id, credentials_path):
        ...
        # 1. AUTENTICACIÓN: Le decimos a Python dónde está la "llave" de Google
        if not os.path.exists(credentials_path):
            logging.error(f"🚨 No se encontró el archivo de credenciales en: {credentials_path}")
        else:
            os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = credentials_path
            
        # La ruta completa hacia tu tabla en la nube
    ...
    def inyectar_datos(self, df):
        ...
        try:
            ...
            # Usamos la función nativa de pandas para inyectar a BigQuery (gbq)
            # if_exists='append' asegura que los datos nuevos se sumen a los viejos sin borrarlos
            df.to_gbq(
